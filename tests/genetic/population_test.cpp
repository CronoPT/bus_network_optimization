...
TEST_F(population_test, population_empty_constructor_test) {
	...
	sol1.set_fitness(0.8);
	sol1.set_total_cost(0.8);
	_population.add_solution(sol1);
	genetic::solution<int> sol2 = genetic::solution<int>(70);
	sol2.set_fitness(0.7);
	sol2.set_total_cost(0.7);
	_population.add_solution(sol2);
	genetic::solution<int> sol3 = genetic::solution<int>(50);
	sol3.set_fitness(0.5);
	sol3.set_total_cost(0.5);
	_population.add_solution(sol3);
	genetic::solution<int> sol4 = genetic::solution<int>(60);
	sol4.set_fitness(0.6);
	sol4.set_total_cost(0.6);
	_population.add_solution(sol4);

	ASSERT_EQ(sol1, _population.get_nth_best(3));
	ASSERT_EQ(sol2, _population.get_nth_best(2));
	ASSERT_EQ(sol3, _population.get_nth_best(0));
	ASSERT_EQ(sol4, _population.get_nth_best(1));
}
...
TEST_F(population_test, population_clip_even_test) {
	...
	ASSERT_EQ(sol3, _population.get_nth_best(0));
	ASSERT_EQ(sol4, _population.get_nth_best(1));
	ASSERT_EQ(4,    _population.get_solutions().size());

	_population.clip();

	ASSERT_EQ(sol3, _population.get_nth_best(0));
	ASSERT_EQ(sol4, _population.get_nth_best(1));
	ASSERT_EQ(2,    _population.get_solutions().size());
}
...
TEST_F(population_test, population_clip_odd_test) {
	...
	ASSERT_EQ(sol1, _population.get_nth_best(2));
	ASSERT_EQ(sol2, _population.get_nth_best(1));
	ASSERT_EQ(sol3, _population.get_nth_best(0));
	ASSERT_EQ(3,    _population.get_solutions().size());
